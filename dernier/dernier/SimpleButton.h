...
ButtonItem* cree_Button(ButtonItem* Data)
{
    ...
    GtkWidget* my_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
    gtk_box_pack_start(GTK_BOX(my_box), label_widget, FALSE, FALSE, 20);

    gtk_container_add(GTK_CONTAINER(Data->widget), my_box);
    ...
    {
        GtkWidget* my_icon;
        my_icon = gtk_image_new_from_icon_name(Data->icon, GTK_ICON_SIZE_BUTTON);
        gtk_box_pack_start(GTK_BOX(my_box), my_icon, FALSE, FALSE, 0);
    }

    //GtkWidget* my_icon = gtk_image_new_from_icon_name("application-exit", GTK_ICON_SIZE_BUTTON); // remplacer "my-icon-name" par le nom de l'icône que vous voulez utiliser
    //gtk_button_set_image(GTK_BUTTON(Data->widget), my_icon);
    //gtk_button_set_image_position(GTK_BUTTON(Data->widget),GTK_POS_RIGHT);
    return Data;
}
...
ButtonItem* set_propriety_ButtonItem(ButtonItem* button, cellule_propriete* liste)
{
    while (liste != NULL)
    {
        ...
        {
            strcpy(button->icon, liste->valeur);
        }
...
void ajouter_Button_container(GtkWidget* container, ButtonItem* button)
{
